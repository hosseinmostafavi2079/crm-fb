...
class NotificationRuleViewSet(viewsets.ModelViewSet):
    ...
    def run_check(self, request):
        ...
        for rule in rules:
            ...
            if rule.service_type == 'warranty':
                devices = Device.objects.filter(warranty_end=target_date)
            else:
                devices = Device.objects.filter(antivirus_expiry=target_date)
            
            for dev in devices:
                # جلوگیری از ارسال تکراری در همان روز
                if SMSLog.objects.filter(device=dev, rule_name=rule.title, sent_at__date=today).exists():
                    continue
                
                msg = rule.template.text if rule.template else "هشدار انقضا"
                msg = msg.replace('{name}', dev.customer.name).replace('{device}', dev.device_name)
                
                send_sms_api(dev.customer.phone, msg)
        ...
        return Response({"message": f"{count} پیامک بر اساس قوانین ارسال شد"})
